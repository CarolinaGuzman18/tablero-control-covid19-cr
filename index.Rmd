...
plot_ly(data = df_general_pais,
  ...
  layout(title = "",
                 ...
                 legend = list(x = 0.1, y = 0.9, language = list(url = '//cdn.datatables.net/plug-ins/1.10.11/i18n/Spanish.json')
                               ),
                 hovermode = "compare") 
...
st_drop_geometry(sf_general_distritos_ultima_fecha) %>% 
  select(Provincia = provincia, Cantón = canton, Distrito = distrito, Positivos = positivos, Activos = activos, Fallecidos = fallecidos, Recuperados = recuperados) %>%
   arrange(desc(Positivos)) %>%
